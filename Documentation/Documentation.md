...
## Folder and File Layout
**Note:** Directory names are **bolded** and file names are *italicized*. Some less relevant directories and files have been omitted (noted in the parent directory).

**Code** (Contains all currently used code)  
├── **Modules** (Contains classes and constants used in *ServerDaemon.py*, *Client.py* and similar scripts)  
│   ├── *DataObjects.py* – A DataObject class for inter-process communication and MessageObject class for messages  
│   ├── *DatabaseManager.py* – A DatabaseManager class handling all database actions (querying, inserting, deleting, etc.)  
│   ├── *Constants.py* – Configuration file with constants used by DataObjects and DatabaseManager classes  
│   ├── *Flags.py* – Defines possible requests and statuses for the DataObject class  
│   └── *SelectorData.py* – A SelectorData class used by selectors in ServerDaemon.py  
├── **User_Data** (Contains Password.db and Message.db databases; .db files are omitted)  
├── **Tests** (Contains tests for Database and DataObjects; test files are omitted)  
├── **Analytics** (Contains plots, test data, and results from *Analytics.py*; test data and results files are omitted)  
│   └── **Plots** (Contains the plots generated by *Analytics.py*)  
├── *Client.py* – Client-side code for the project  
├── *ServerDaemon.py* – Server-side code for the project  
├── *TerminalClient.py* – Terminal-based client for debugging  
└── *Analytics.py* – Quantifies the difference between Custom and JSON Wire Protocols  

**Documentation** (Contains the Engineering Notebook, notes on Code Review Groups, and this documentation)  
├── *Documentation.md*  
├── *engineering_notebook.md*  
└── *Group_notes.md*  

**Old_Versions** (Repository for older versions of the client and server code; subdirectories and files omitted)  

**venv** (Virtual Python Environment; subdirectories and files omitted)
